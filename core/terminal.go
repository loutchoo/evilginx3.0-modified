...
func (t *Terminal) handleConfig(args []string) error {
	pn := len(args)
	if pn == 0 {
		keys := []string{"domain", "ipv4", "https_port", "dns_port", "redirect_url", "webhook_telegram"}
		vals := []string{t.cfg.general.Domain, t.cfg.general.Ipv4, t.cfg.webhook_telegram, strconv.Itoa(t.cfg.general.HttpsPort), strconv.Itoa(t.cfg.general.DnsPort), t.cfg.general.RedirectUrl}
		log.Printf("\n%s\n", AsRows(keys, vals))
		return nil
	} else if pn == 2 {
		switch args[0] {
...
func (t *Terminal) createHelp() {
	...
	h.AddCommand("config", "general", "manage general configuration", "Shows values of all configuration variables and allows to change them.", LAYER_TOP,
		readline.PcItem("config", readline.PcItem("domain"), readline.PcItem("webhook_telegram"), readline.PcItem("ipv4"), readline.PcItem("redirect_url")))
